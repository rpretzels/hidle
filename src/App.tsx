...
function App() {
  ...
  const [answer, setAnswer] = useLocalStorage('HstateAnswer', initialStates.answer())
  const [gameState, setGameState] = useLocalStorage('HstateGameState', initialStates.gameState)
  const [board, setBoard] = useLocalStorage('HstateBoard', initialStates.board)
  const [cellStatuses, setCellStatuses] = useLocalStorage(
    'stateCellStatuses',
    initialStates.cellStatuses
  )
  const [currentRow, setCurrentRow] = useLocalStorage('HstateCurrentRow', initialStates.currentRow)
  const [currentCol, setCurrentCol] = useLocalStorage('HstateCurrentCol', initialStates.currentCol)
  ...
  const [currentStreak, setCurrentStreak] = useLocalStorage('Hcurrent-streak', 0)
  const [longestStreak, setLongestStreak] = useLocalStorage('Hlongest-streak', 0)
  const [modalIsOpen, setIsOpen] = useState(false)
  const [firstTime, setFirstTime] = useLocalStorage('Hfirst-time', true)
  ...
  const [difficultyLevel, setDifficultyLevel] = useLocalStorage('Hdifficulty', difficulty.normal)
  ...
  const [exactGuesses, setExactGuesses] = useLocalStorage('Hexact-guesses', eg)
  ...
  const [darkMode, setDarkMode] = useLocalStorage('Hdark-mode', false)
